...
nclimgrid_prcptotSdev_ann = np.array(nclimgrid_data.variables['prcptotSdev_ann'][0,:,:])
mask = np.isclose(nclimgrid_prcptotSdev_ann,1e+20)
nclimgrid_prcptotSdev_ann[mask] = np.nan
lat = np.array(nclimgrid_data.variables['lat'])
lon = np.array(nclimgrid_data.variables['lon'])
...
c = plt.contourf(lon,lat,nclimgrid_prcptotSdev_ann,levels,transform=ccrs.PlateCarree(),cmap=get_cmap("jet_r"),extend='both')
plt.savefig(image_dir+'nclimgrid_prcptotSdev_historical_mean_blank.png',dpi=300,bbox_inches='tight', pad_inches=0,transparent=True)
plt.show()
plt.close()


star_ens_data = nc.Dataset("/Volumes/Elements/GFDL_project/DOWNSCALING/SUBPROJECTS/LOCA2vsSTAR/diagnostics/Climdex/ClimdexPhard/Ensembles/STAR/1985-2014/prcptot_EnsembleStats_ClimdexPhard-ann_STAR_NCA16_historical_CONUS16thD2021_1985-2014_30yr_Sdev_ann.nc")
star_ens_prcptot_sdev = np.array(star_ens_data.variables['prcptotSdev_annmean'][0,:,:])
mask = np.isclose(star_ens_prcptot_sdev,1e+20)
star_ens_prcptot_sdev[mask] = np.nan

fig = plt.figure(figsize=(14,6))
ax = fig.add_subplot(1,1,1,projection=ccrs.Mercator())
ax.set_extent([-125,-65,24,50],crs=ccrs.PlateCarree())
c = plt.contourf(lon,lat,star_ens_prcptot_sdev,levels,transform=ccrs.PlateCarree(),cmap=get_cmap("jet_r"),extend='both')
plt.savefig(image_dir+'star_prcptotSdev_historical_mean_blank.png',dpi=300,bbox_inches='tight', pad_inches=0,transparent=True)
plt.show()
plt.close()


bias = star_ens_prcptot_sdev/nclimgrid_prcptotSdev_ann
bias_levels = np.arange(0.5,1.51,0.01)
...
plt.savefig(image_dir+'nclimgrid_star_prcptotSDEV_historical_bias_blank_v2.png',dpi=300,bbox_inches='tight', pad_inches=0,transparent=True)
plt.show()
plt.close()
...
for i in range(7):
    globals()['nclimgrid_'+str(regions_names_alt[i])+'_timeseries_sdev'] = np.nanstd(globals()['nclimgrid_'+str(regions_names_alt[i])],axis=(1,2))
...
for i in range(7):
    globals()['livneh_'+str(regions_names_alt[i])+'_timeseries_sdev'] = np.nanstd(globals()['livneh_'+str(regions_names_alt[i])],axis=(1,2))
...
for i in range(7):
    for j in range(16):
        globals()['star_'+str(models[j])+str(regions_names_alt[i])+'_timeseries_std'] = np.nanstd(globals()['star_'+str(models[j])+str(regions_names_alt[i])],axis=(1,2))

ensemble = np.empty([7,16,30])
for i in range(7):
    for j in range(16):
        ensemble[i,j,:] = globals()['star_'+str(models[j])+str(regions_names_alt[i])+'_timeseries_std']
...
for h in range(7):
    ...
    for i in range(16):
        if i in {0}:
            plt.plot(np.arange(1985,2015,1),globals()['star_'+str(models[i])+str(regions_names_alt[h])+'_timeseries_std'],color='grey',ls='--',alpha=0.75,label='STAR',lw=1)
        else:
            plt.plot(np.arange(1985,2015,1),globals()['star_'+str(models[i])+str(regions_names_alt[h])+'_timeseries_std'],color='grey',ls='--',alpha=0.75,lw=1)
    plt.plot(np.arange(1985,2015,1),ensemble_avg[h,:],lw=3,label='Ens. Avg.',ls='--',color='k')
    plt.plot(np.arange(1985,2015,1),globals()['livneh_'+str(regions_names_alt[h])+'_timeseries_sdev'],lw=3,color='tab:orange',label='Livneh')
    plt.plot(np.arange(1985,2015,1),globals()['nclimgrid_'+str(regions_names_alt[h])+'_timeseries_sdev'],lw=3,label='nClimGrid')
    plt.legend()
    plt.xlim(1984.5,2014.5)
    plt.ylabel("Avg. Annual Total Precip. (mm)")
    plt.xticks(size=12)
    plt.savefig(image_dir+'prcptotSDEV_time_series_obs_STAR_'+str(regions_names_alt[h])+'.png',dpi=300,bbox_inches='tight', pad_inches=0)
    plt.show()
    plt.close()
...
livneh_prcptotSdev_ann = np.array(livneh_data.variables['prcptotSdev_ann'][0,:,:])
mask = np.isclose(livneh_prcptotSdev_ann,1e+20)
livneh_prcptotSdev_ann[mask] = np.nan
mask = np.isnan(nclimgrid_prcptotSdev_ann)
livneh_prcptotSdev_ann[mask] = np.nan
lat = np.array(livneh_data.variables['lat'])
lon = np.array(livneh_data.variables['lon'])
...
ax = fig.add_subplot(1,1,1,projection=ccrs.Mercator())
ax.set_extent([-125,-65,24,50],crs=ccrs.PlateCarree())
c = plt.contourf(lon,lat,livneh_prcptotSdev_ann,levels,transform=ccrs.PlateCarree(),cmap=get_cmap("jet_r"),extend='both')
plt.savefig(image_dir+'livneh_prcptotSdev_historical_mean_blank.png',dpi=300,bbox_inches='tight', pad_inches=0,transparent=True)
plt.show()
plt.close()


loca_ens_data = nc.Dataset("/Volumes/Elements/GFDL_project/DOWNSCALING/SUBPROJECTS/LOCA2vsSTAR/diagnostics/Climdex/ClimdexPhard/Ensembles/LOCA2/1985-2014/prcptot_EnsembleStats_ClimdexPhard-ann_LOCA2_NCA16_historical_CONUS16thD2021_1985-2014_30yr_Sdev_ann.nc")
loca_ens_prcptot_sdev = np.array(loca_ens_data.variables['prcptotSdev_annmean'][0,:,:])
mask = np.isclose(loca_ens_prcptot_sdev,1e+20)
loca_ens_prcptot_sdev[mask] = np.nan
mask = np.isnan(nclimgrid_prcptotSdev_ann)
loca_ens_prcptot_sdev[mask] = np.nan
...
ax = fig.add_subplot(1,1,1,projection=ccrs.Mercator())
ax.set_extent([-125,-65,24,50],crs=ccrs.PlateCarree())
c = plt.contourf(lon,lat,loca_ens_prcptot_sdev,levels,transform=ccrs.PlateCarree(),cmap=get_cmap("jet_r"),extend='both')
plt.savefig(image_dir+'loca_prcptotSdev_historical_mean_blank.png',dpi=300,bbox_inches='tight', pad_inches=0,transparent=True)
plt.show()
plt.close()

bias = loca_ens_prcptot_sdev/livneh_prcptotSdev_ann

fig = plt.figure(figsize=(14,6))
ax = fig.add_subplot(1,1,1,projection=ccrs.Mercator())
ax.set_extent([-125,-65,24,50],crs=ccrs.PlateCarree())
c = plt.contourf(lon,lat,bias,bias_levels,transform=ccrs.PlateCarree(),cmap=custom_cmap,extend='both')
plt.savefig(image_dir+'livneh_loca_prcptotSDEV_historical_bias_blank_v2.png',dpi=300,bbox_inches='tight', pad_inches=0,transparent=True)
plt.show()
plt.close()
...
for i in range(7):
    for j in range(16):
        globals()['loca_'+str(models[j])+str(regions_names_alt[i])+'_timeseries_std'] = np.nanstd(globals()['loca_'+str(models[j])+str(regions_names_alt[i])],axis=(1,2))


ensemble = np.empty([7,16,30])
for i in range(7):
    for j in range(16):
        ensemble[i,j,:] = globals()['loca_'+str(models[j])+str(regions_names_alt[i])+'_timeseries_std']
...
for h in range(7):
    ...
    for i in range(16):
        if i in {0}:
            plt.plot(np.arange(1985,2015,1),globals()['loca_'+str(models[i])+str(regions_names_alt[h])+'_timeseries_std'],color='grey',ls='--',alpha=0.75,label='LOCA2',lw=1)
        else:
            plt.plot(np.arange(1985,2015,1),globals()['loca_'+str(models[i])+str(regions_names_alt[h])+'_timeseries_std'],color='grey',ls='--',alpha=0.75,lw=1)
    plt.plot(np.arange(1985,2015,1),ensemble_avg[h,:],lw=3,label='Ens. Avg.',ls='--',color='k')
    plt.plot(np.arange(1985,2015,1),globals()['livneh_'+str(regions_names_alt[h])+'_timeseries_sdev'],lw=3,color='tab:orange',label='Livneh')
    plt.plot(np.arange(1985,2015,1),globals()['nclimgrid_'+str(regions_names_alt[h])+'_timeseries_sdev'],lw=3,label='nClimGrid')
    plt.legend()
    ...
    plt.ylabel("Avg. Annual Total Precip. (mm)")
    plt.xticks(size=12)
    plt.savefig(image_dir+'prcptotSDEV_time_series_obs_LOCA2_'+str(regions_names_alt[h])+'.png',dpi=300,bbox_inches='tight', pad_inches=0)
    plt.show()
    plt.close()
