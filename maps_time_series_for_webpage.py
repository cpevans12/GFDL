...
# cbar = fig.colorbar(c, orientation='horizontal', shrink=0.55,pad=0.05)
# cbar.set_label(r'$\frac{STAR}{nClimGrid}$',fontsize=15)
...
print("Masking took", time.time() - start, "seconds")
...
# for i in range(16):
#     if i in {0}:
#         plt.plot(np.arange(1985,2015,1),globals()['star_'+str(models[i])+str(regions_names_alt[0])+'_timeseries'],color='grey',ls='--',alpha=0.75,label='STAR',lw=1)
#     else:
#         plt.plot(np.arange(1985,2015,1),globals()['star_'+str(models[i])+str(regions_names_alt[0])+'_timeseries'],color='grey',ls='--',alpha=0.75,lw=1)
# plt.plot(np.arange(1985,2015,1),globals()['livneh_'+str(regions_names_alt[0])+'_timeseries'],lw=3,color='tab:orange',label='Livneh')
# plt.plot(np.arange(1985,2015,1),globals()['nclimgrid_'+str(regions_names_alt[0])+'_timeseries'],lw=3,label='nClimGrid')
# plt.legend()
# plt.xlim(1984.5,2014.5)
# plt.ylabel("Avg. Annual Total Precip. (mm)")
# plt.xticks(size=12)
# plt.savefig(image_dir+'prcptot_time_series_obs_STAR_'+str(regions_names_alt[0])+'.png',dpi=300,bbox_inches='tight', pad_inches=0)
# plt.show()
# plt.close()
